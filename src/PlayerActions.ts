...
export default class PlayerActions {
  ...
  public startPlayerEventListener(client: any) {
    ...
    //Add playlist
    client.player.on("addList", (queue: any, playlist: any) =>
    ...
    //Error
    client.player.on("error", (channel: any, error: any) =>
      channel.send(
    ...
    client.player.on("noRelated", (queue: any) =>
      queue.textChannel.send(
    ...
    client.player.on("searchCancel", (message: any) =>
    ...
    client.player.on("searchInvalidAnswer", (message: any) =>
      message.channel.send(
    ...
    client.player.on("searchNoResult", (message: any, query: any) =>
      message.channel.send(
  ...
  private addSong(client: any) {
    client.player.on("addSong", (queue: any, song: any) =>
      queue.textChannel.send({
        embeds: [
          new MessageEmbed()
  ...
  private playSong(client: any) {
    client.player.on("playSong", (queue: any, song: any) =>
      queue.textChannel.send({
        embeds: [
          new MessageEmbed()
            .setColor(this.assets.embedColor)
  ...
  private empty(client: any) {
    client.player.on("empty", (queue: any) =>
      queue.textChannel.send({
        embeds: [
          new MessageEmbed()
            .setColor(this.assets.embedColor)
            .setDescription(
              `${this.assets.errorEmoji}  |  Channel is empty, leaving all channels!`
            )
            .setAuthor({
              name: this.assets.name,
              iconURL: this.assets.logoPFP6,
  ...
  private finish(client: any) {
    client.player.on("finish", (queue: any) =>
      queue.textChannel
        .send({
          embeds: [
            new MessageEmbed()
              .setColor(this.assets.embedColor)
              .setDescription(
                `${this.assets.errorEmoji}  |  Queue has finished! In order to keep using Alive Music Bot, add some more songs to the queue!`
              )
              .setAuthor({
                name: this.assets.name,
                iconURL: this.assets.logoPFP6,
        ...
        .then((repliedMessage: any) => {
          setTimeout(
            () => repliedMessage.delete(),
            this.assets.deleteDurationNP
          );
        })
    );
  }
