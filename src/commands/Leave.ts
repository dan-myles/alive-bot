...
export default class Leave {
  ...
  constructor() {
    ...
    this.data = new SlashCommandBuilder()
      .setName("leave")
      .setDescription(
        "Disconnects Alive Music Bot from all voice channels and clears the queue"
      );
  }
  ...
  public async execute(interaction: any, client: any) {
    ...
    if (typeof queue === "undefined") {
      ...
      if (voiceChannel) {
        ...
        this.logger.warn(
          "Failed executing /leave command: USER VOICE CHANNEL NOT FOUND"
        );
      }
    } else {
      ...
      if (voiceChannel) {
        ...
        if (userId === botId) {
          ...
          this.logger.warn(
            "Failed executing /leave command: USER AND APPLICATION VOICE IDS DO NOT MATCH"
          );
        }
      } else {
        //User is not in a voice channel
