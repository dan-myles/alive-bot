  ...
  public async execute(interaction: any, client: any) {
    ...
    if (typeof queue === "undefined") {
      ...
      if (voiceChannel) {
        ...
        this.logger.warn(
          "Failed executing /autoplay command: PLAYER NOT FOUND"
        );
      } else {
        ...
        this.logger.warn(
          "Failed executing /autoplay command: USER VOICE CHANNEL NOT FOUND"
        );
      }
    } else {
      ...
      if (voiceChannel) {
        ...
        if (userId === botId) {
          ...
          await interaction.reply({
            embeds: [
              {
                description: `Auto-Play has been turned: \`${
                  autoPlay ? "On" : "Off"
                }\``,
          ...
          this.logger.warn(
            "Failed executing /autoplay command: USER AND APPLICATION VOICE IDS DO NOT MATCH"
          );
        }
      } else {
        //User is not in a voice channel
